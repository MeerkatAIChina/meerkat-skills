...
description: "Analyze and optimize tweets for maximum reach using Twitter's open-source algorithm insights (Real-graph, SimClusters, TwHIN, Tweepcred). Rewrite and edit tweets to improve engagement and visibility based on how the recommendation system ranks content."
metadata:
  version: 1.0.0
  author: MeerkatAIChina (sourced from ComposioHQ/awesome-claude-skills)
  license: MIT
  hermes:
    tags: [social-media, twitter, content-optimization, algorithm]
    related_skills: [social-content, social-media-strategy, brand-voice-guidelines, copywriting-frameworks]
    optimized: true
